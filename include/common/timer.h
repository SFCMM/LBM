...
public:
  ...
  [[nodiscard]] auto inline recordedTime() const -> GDouble {
    return m_recordedTime;
  }

  auto inline subTimers() -> std::vector<GInt> & { return m_subTimers; }
  [[nodiscard]] auto inline subTimers() const -> const std::vector<GInt> & {
    return m_subTimers;
  }

  [[nodiscard]] auto inline oldCpuTime() const -> const chronoTimePoint & {
    return m_oldCpuTime;
  }

  auto inline cpuTime() -> chronoTimePoint & { return m_cpuTime; }
  [[nodiscard]] auto inline cpuTime() const -> const chronoTimePoint & {
    return m_cpuTime;
  }
...
public:
  ...
  inline auto newGroupTimer(const std::string &timerName, const GInt groupId)
      -> GInt;
  inline auto newSubTimer(const std::string &timerName, const GInt timerId)
      -> GInt;
  ...
  inline void recordTimerStart(const GInt timerId,
                               const GString &pos = ""); // reset + start
  inline void recordTimerStop(const GInt timerId,
                              const GString &pos = ""); // stop + record
...
private:
  ...
  [[nodiscard]] static inline auto indent(const GInt pIndent) -> GInt {
    return pIndent + 2;
  };
};
...
inline void TimerManager::resetRecord(const GInt timerId) {
...
inline void TimerManager::recordTimerStop(const GInt timerId,
                                          const GString &pos) {
  if (timerId < 0) {
    return;
  }
...
inline void TimerManager::recordTimer(const GInt timerId) {
...
inline auto TimerManager::newGroupTimer(const std::string &name,
                                        const GInt groupId) -> GInt {
...
/// Creates a new timer and returns its timerId.
inline auto TimerManager::newSubTimer(const std::string &name,
                                      const GInt timerId) -> GInt {
  if (timerId < 0) {
    return -1;
  }

  ASSERT(static_cast<std::size_t>(timerId) < m_timers.size(),
         "timerId " + std::to_string(timerId) +
             " does not exist when trying to create subtimer with name " +
             name);
...
inline void TimerManager::recordTimerStart(const GInt timerId,
                                           const GString &pos) {
  if (timerId < 0) {
    return;
  }
...
inline auto TimerManager::returnTimerTime(const GInt timerId) -> GDouble {
...
inline void TimerManager::stopTimer(const GInt timerId, const GString &pos) {
  ...
  if (m_timers[timerId].status() == Timer::Running) {
    ...
    const GString msg = "The timer '" + m_timers[timerId].name() +
...
inline void TimerManager::displayTimer_(const GInt timerId,
  ...
  indentedName << "[" << std::fixed << std::setprecision(1) << std::setw(4)
               << std::setfill('0') << std::right << percentage << std::left
               << "%] ";
  ...
  logger << std::endl;
  for (std::size_t sub = 0, last = m_timers[timerId].subTimers().size();
       sub < last; ++sub) {
    const GInt new_indent = indent(tIndent);
    displayTimer_(m_timers[timerId].subTimers()[sub], toggleDisplayed,
                  new_indent, m_timers[timerId].recordedTime());
  }
  if (running) {
    startTimer(timerId);
  }
...
inline void TimerManager::displayAllTimers(const GInt groupId) {
  ASSERT(static_cast<GInt>(m_timers.size()) > 0,
...
inline void NEW_TIMER_GROUP_NOCREATE(GInt &id, const GString &groupName) {
  id = timers().newGroup(groupName);
}

inline void NEW_TIMER_NOCREATE(GInt &id, const GString &timerName,
                               const GInt groupId) {
  id = timers().newGroupTimer(timerName, groupId);
}

inline void NEW_SUB_TIMER_NOCREATE(GInt &id, const GString &timerName,
                                   const GInt timerId) {
...
inline void RETURN_TIMER_TIME(const GInt timerId) {
  timers().returnTimerTime(timerId);
}
...
inline void DISPLAY_TIMER(const GInt timerId) {
  timers().displayTimer(timerId);
}

inline void DISPLAY_TIMER_INTERM(const GInt timerId) {
  timers().displayTimerNoToggleDisplayed(timerId);
}

inline void DISPLAY_ALL_GROUP_TIMERS(const GInt groupId) {
  timers().displayAllTimers(groupId);
}
...
// Currently, not used.
//#define NEW_TIMER_GROUP(id, groupName)                 const GInt id =
// timers().newGroup(groupName) #define NEW_TIMER(id, timerName, groupId) const
// GInt id = timers().newGroupTimer(timerName, groupId) #define
// NEW_SUB_TIMER(id, timerName, timerId)          const GInt id =
// timers().newSubTimer(timerName, timerId) #define NEW_TIMER_GROUP_STATIC(id,
// groupName)          static const GInt id = timers().newGroup(groupName)
// #define NEW_TIMER_STATIC(id, timerName, groupId)       static const GInt id =
// timers().newGroupTimer(timerName, groupId) #define NEW_SUB_TIMER_STATIC(id,
// timerName, timerId)   static const GInt id = timers().newSubTimer(timerName,
// timerId)
...
public:
  explicit FunctionTiming(std::string name)
      : m_initCpuTime(cpuTime()), m_deltaCpuTime(0), m_tmpCpuTime(0),
        m_initWallTime(wallTime()), m_deltaWallTime(0.0), m_tmpWallTime(-1.0),
  ...
  [[nodiscard]] auto getDeltaCpuTime() const -> clock_t {
    return m_deltaCpuTime;
  }
  [[nodiscard]] auto getInitWallTime() const -> GDouble {
    return m_initWallTime;
  }
  [[nodiscard]] auto getDeltaWallTime() const -> GDouble {
    return m_deltaWallTime;
  }
...
inline auto operator<(const FunctionTiming &a, const FunctionTiming &b)
    -> GBool {
...
public:
  explicit TimerProfiling(std::string name)
      : m_initCpuTime(cpuTime()), m_initWallTime(wallTime()),
        m_name(std::move(name)) {}
  ~TimerProfiling() {
    using namespace std;
    ...
    sstream << "    CPU      WALL   FUNCTION                    >> profile: '"
            << m_name << "' <<";
    ...
    if (!s_functionTimings.empty()) {
      ...
      for (auto &s_functionTiming : s_functionTimings) {
        ...
        const GDouble relCpuTime =
            100.0 * getCpuTimeSecs(s_functionTiming.getDeltaCpuTime()) /
            max(1e-15, getCpuTimeSecs(exitCpuTime - m_initCpuTime));
        const GDouble relWallTime = 100.0 *
                                    s_functionTiming.getDeltaWallTime() /
                                    max(1e-15, (exitWallTime - m_initWallTime));
        if (relCpuTime < thresholdPercentage) {
          supCounter++;
          continue;
        }
        stringstream ss;
        ss << fixed << setprecision(2) << relCpuTime << "%   " << relWallTime
           << "%   " << s_functionTiming.getName();
        logger << ss.str() << endl;
        counter++;
      }
      if (supCounter > 0) {
    ...
    logger << endl;
    logger << "Total cpu time:  "
           << printTime(getCpuTimeSecs(exitCpuTime - m_initCpuTime)) << endl;
    logger << "Total wall time: " << printTime(exitWallTime - m_initWallTime)
           << endl;
    for (std::size_t i = 0; i < header.size(); i++) {
      logger << "_";
    }
  ...
  static auto getTimingId(const std::string &name) -> GInt {
    GInt tId = -1;
    if (!s_functionTimings.empty()) {
      for (std::vector<FunctionTiming>::size_type i = 0;
           i < s_functionTimings.size(); i++) {
...
class Tracer {
public:
  Tracer(const GString &funloc)
      : m_timerId(TimerProfiling::getTimingId(funloc)) {
    TimerProfiling::s_functionTimings[m_timerId].in();
  }
...
#define PROFILE()                                                              \
  profiling::Tracer USE_ONLY_ONCE_PER_SCOPE(__FUNCTION_LOCATION__)
