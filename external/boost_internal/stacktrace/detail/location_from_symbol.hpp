...
#   include <dlfcn.h>
#else
#   include <boost_internal/winapi/dll.hpp>
#endif
...
class location_from_symbol {
    BOOST_STATIC_CONSTEXPR boost_internal::winapi::DWORD_ DEFAULT_PATH_SIZE_ = 260;
...
public:
    explicit location_from_symbol(const void* addr) BOOST_NOEXCEPT {
        ...
        boost_internal::winapi::MEMORY_BASIC_INFORMATION_ mbi;
        if (!boost_internal::winapi::VirtualQuery(addr, &mbi, sizeof(mbi))) {
            return;
        }

        boost_internal::winapi::HMODULE_ handle = reinterpret_cast<boost_internal::winapi::HMODULE_>(mbi.AllocationBase);
        if (!boost_internal::winapi::GetModuleFileNameA(handle, file_name_, DEFAULT_PATH_SIZE_)) {
            file_name_[0] = '\0';
            return;
        }
    }
...
public:
    program_location() BOOST_NOEXCEPT {
        ...
        const boost_internal::winapi::HMODULE_ handle = 0;
        if (!boost_internal::winapi::GetModuleFileNameA(handle, file_name_, DEFAULT_PATH_SIZE_)) {
            file_name_[0] = '\0';
        }
    }
