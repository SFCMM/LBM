...
//#include <boost_internal/winapi/config.hpp>
//#endif
...
/// so you could read them by using 'od -tx8 -An stacktrace_dump_failename' Linux command or using boost_internal::stacktrace::stacktrace::from_dump functions.
...
///
/// @returns Stored call sequence depth including terminating zero frame. To get the actually consumed bytes multiply this value by the sizeof(boost_internal::stacktrace::frame::native_frame_ptr_t)
///
...
BOOST_FORCEINLINE std::size_t safe_dump_to(void* fd) BOOST_NOEXCEPT {
  return boost_internal::stacktrace::detail::this_thread_frames::safe_dump_to_impl(fd, 0, boost_internal::stacktrace::detail::max_frames_dump);
}

BOOST_FORCEINLINE std::size_t safe_dump_to(std::size_t skip, std::size_t max_depth, void* fd) BOOST_NOEXCEPT {
  return boost_internal::stacktrace::detail::this_thread_frames::safe_dump_to_impl(fd, skip, max_depth);
}
...
#       include <boost_internal/stacktrace/detail/safe_dump_noop.ipp>
#       include <boost_internal/stacktrace/detail/collect_noop.ipp>
#   else
#       if defined(BOOST_WINDOWS)
#           include <boost_internal/stacktrace/detail/safe_dump_win.ipp>
#       else
...
#           include <boost_internal/stacktrace/detail/collect_msvc.ipp>
#       else
