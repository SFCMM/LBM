...
//#include <boost_internal/config.hpp>
//#ifdef BOOST_HAS_PRAGMA_ONCE
//#   pragma once
//#endif

//#include <boost_internal/core/explicit_operator_bool.hpp>
//#include <boost_internal/core/no_exceptions_support.hpp>
//#include <boost_internal/container_hash/hash_fwd.hpp>
...
#include "boost_internal/stacktrace/stacktrace_fwd.h"
#include "boost_internal/stacktrace/safe_dump_to.h"
#include "boost_internal/stacktrace/detail/frame_decl.hpp"
...
class basic_stacktrace {
  ...
  BOOST_NOINLINE void init(std::size_t frames_to_skip, std::size_t max_depth) {
    ...
    { if (true) {
      { // Fast path without additional allocations
        ...
        const std::size_t frames_count =
            boost::stacktrace::detail::this_thread_frames::collect(
                buffer, buffer_size < max_depth ? buffer_size : max_depth,
                frames_to_skip + 1);
      ...
      typedef typename Allocator::template rebind<native_frame_ptr_t>::other
          allocator_void_t;
#else
      typedef typename std::allocator_traits<Allocator>::template rebind_alloc<
          ...
          allocator_void_t;
#endif
      std::vector<native_frame_ptr_t, allocator_void_t> buf(
          buffer_size * 2, 0, impl_.get_allocator());
      do {
        const std::size_t frames_count =
            boost::stacktrace::detail::this_thread_frames::collect(
                &buf[0], buf.size() < max_depth ? buf.size() : max_depth,
                frames_to_skip + 1);
...
public:
  ...
  ///
  /// @param begin Begining of the memory where the stacktrace was saved using the boost_internal::stacktrace::safe_dump_to
  ///
  /// @param buffer_size_in_bytes Size of the memory. Usually the same value that was passed to the boost_internal::stacktrace::safe_dump_to
  ///
...
/// This is the typedef to use unless you'd like to provide a specific allocator to boost_internal::stacktrace::basic_stacktrace.
typedef basic_stacktrace<> stacktrace;

}} // namespace boost_internal::stacktrace
